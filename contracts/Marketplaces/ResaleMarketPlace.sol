...
contract Resale_MarketPlace is ERC721Holder, AccessControl {
  ...
  function onlyItemOwner(address tokenAddress, address addr, uint256 tokenId) public view returns (bool) {
    RAIR_ERC721 itemToken = RAIR_ERC721(tokenAddress);
    return itemToken.ownerOf(tokenId) == addr;
  }

  function setTradeRoyaltyReceivers(uint256 trade, address[] calldata recipients, uint256[] calldata percentages) external OnlyTokenCreator(trades[trade].tokenAddress) {
    require(
  ...
  */
  function executeTrade(uint256 _trade) public payable {
    ...
    TradeRoyaltyReceivers storage tradeRoyaltyReceivers = royaltyReceivers[_trade];
    if (tradeRoyaltyReceivers.recipients.length > 0) {
      for (uint256 i = 0; i < tradeRoyaltyReceivers.recipients.length; i++) {
        uint256 toReceiver = (royalty * tradeRoyaltyReceivers.percentages[i]) / 100000;
        payable(tradeRoyaltyReceivers.recipients[i]).transfer(toReceiver);
      }
    } else {
